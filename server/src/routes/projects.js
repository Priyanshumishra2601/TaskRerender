...
/**
 * GET /projects - Fetch projects the user is part of
 */
router.get("/", async (req, res) => {
  try {
    const userId = req.user.id;
    // Query fixed: 'title' ki jagah 'name' use kiya
    const rows = await query(
      `SELECT DISTINCT p.id, p.name, p.description, p.createdBy, p.createdAt
       FROM projects p
    ...
    const mapped = rows.map((r) => ({
      ...r,
      title: r.name, // Frontend compatibility ke liye
      techStack: [], 
      status: "active"
    }));
...
router.post("/", async (req, res) => {
  try {
    ...
    // CRITICAL FIX: Query matches your DB columns (id, name, description, createdBy, createdAt)
    ...
    const rows = await query(
      `SELECT id, name, description, createdBy, createdAt FROM projects WHERE id = ?`,
      [projectId]
    );
