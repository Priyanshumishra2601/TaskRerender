...
/**
 * Helper: Get all project IDs a user has access to
 */
async function userProjectIds(userId) {
  const rows = await query(
    `SELECT p.id FROM projects p
...
/**
 * GET /tasks - Fetch tasks for projects the user belongs to
 */
router.get("/", async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    // FIX: p.title changed to p.name to match image_ad6840.png
    // Removed t.dueDate and t.completedAt to avoid "Unknown column" errors
    const rows = await query(
      `SELECT t.id, t.projectId, t.assignedTo, t.title, t.status, t.createdAt,
...
router.post("/", requireRole("Admin"), async (req, res) => {
  try {
    ...
    // FIX: Removed dueDate from INSERT as it might be missing in DB
    const result = await query(
      `INSERT INTO tasks (projectId, assignedTo, title, status) VALUES (?, ?, ?, ?)`,
      [projectId, assignee, title, "Pending"]
    );

    const taskId = result.insertId;

    // FIX: p.title changed to p.name
    const rows = await query(
      `SELECT t.id, t.projectId, t.assignedTo, t.title, t.status, t.createdAt,
              p.name AS projectTitle,
              u.name AS assigneeName
...
router.patch("/:id/status", async (req, res) => {
  try {
    ...
    // UPDATE query simplified (Removed completedAt if missing in DB)
    await query(
